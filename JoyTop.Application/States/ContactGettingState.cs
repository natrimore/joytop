    ...
    public class ContactGettingState : IState
    {
        ...
        public async Task Execute(Message message, ITelegramBotClient botClient, CancellationToken cancellationToken = default)
        {
            ...
            else if (message.Text == "1" || message.Text == "2" || message.Text == "3")
            {
                ...
                await ExecuteNextState(message, botClient, cancellationToken);
            }
            else
            {
        ...
        private async Task ExecuteNextState(Message message, ITelegramBotClient botClient, CancellationToken cancellationToken)
        {
            // Implement the logic for the next state based on the user's language
            // For now, just sending a generic message
            await botClient.SendTextMessageAsync(message.Chat.Id,
                $"Language set to {userLanguage}. Proceeding to the next state...",
                cancellationToken: cancellationToken);
        }
    }
